...
class DataProcessor {
  ...
  // Clean phone format with improved international support
  cleanPhone(phone) {
    if (!phone) return "";

    let cleaned = phone.replace(/[^\d+\s()-]/g, "").trim();

    // US/Canada phone format
    if (cleaned.match(/^\+?1?\s*\(?(\d{3})\)?[-.\s]?(\d{3})[-.\s]?(\d{4})$/)) {
    ...
    // UK phone format
    if (cleaned.match(/^\+44\s*(\d{4})\s*(\d{6})$/)) {
      return cleaned.replace(/^\+44\s*(\d{4})\s*(\d{6})$/, "+44 $1 $2");
    }

    // International format - keep as is if it looks valid
    if (cleaned.match(/^\+\d{7,15}$/)) {
      return cleaned;
    }

    // Generic 10-11 digit format
  ...
  removeDuplicates(leads) {
    ...
    leads.forEach((lead) => {
      ...
      const company = this.cleanCompanyName(lead.company);
